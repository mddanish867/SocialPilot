  ...
  return (
    <section className="w-full bg-black text-white overflow-hidden">
      <div className="relative mx-auto">
        ...
        <div className="container mx-auto px-4 md:px-6 py-12 md:py-20 lg:py-28 relative z-10">
          <div className="grid lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16 xl:gap-24 items-center">
            {/* Content Column */}
            <div className={`space-y-6 md:space-y-8 transform ${isLoaded ? 'translate-y-0 opacity-100' : 'translate-y-12 opacity-0'} transition-all duration-1000 ease-out`}>
              {/* Platform tabs - horizontally scrollable on small screens */}
                ...
                <div className="flex space-x-2 sm:space-x-4">
                  {Object.entries(platforms).map(([key, platform]) => (
                    <button 
                      ...
                      className={`flex-shrink-0 flex items-center px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium transition-all duration-300 ${activeTab === key ? `bg-gradient-to-r ${platform.color} shadow-lg` : 'bg-white/10 hover:bg-white/20'}`}
                    >
              ...
              {/* Metrics with responsive grid layout */}
              <div className="grid grid-cols-3 gap-2 sm:gap-4 py-4 sm:py-6 border-y border-white/10">
                <div>
                  <div className="text-xl sm:text-2xl md:text-3xl font-bold">3x</div>
                  <div className="text-xs sm:text-sm text-gray-400">Faster Growth</div>
                </div>
                <div>
                  <div className="text-xl sm:text-2xl md:text-3xl font-bold">47%</div>
                  <div className="text-xs sm:text-sm text-gray-400">More Engagement</div>
                </div>
                <div>
              ...
              {/* CTA section - stacked on mobile, side by side on larger screens */}
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <button className="relative overflow-hidden group px-6 sm:px-8 py-3 sm:py-4 bg-white hover:bg-gray-100 text-black font-bold rounded-lg transition-all duration-300">
                ...
                <button className="px-6 sm:px-8 py-3 sm:py-4 bg-white/10 hover:bg-white/20 font-medium rounded-lg backdrop-blur-sm transition-all duration-300">
            ...
            {/* Dashboard Preview Column - hidden on smallest screens, shows up from sm breakpoint */}
            <div className={`relative transform perspective-1000 ${isLoaded ? 'translate-y-0 opacity-100 rotate-0' : 'translate-y-12 opacity-0 rotate-6'} transition-all duration-1000 delay-300 ease-out mt-8 lg:mt-0`}>
                ...
                <div className="relative bg-gray-900 border border-gray-800 rounded-xl overflow-hidden shadow-2xl">
                  ...
                  <div className="flex items-center justify-between px-3 sm:px-4 py-2 sm:py-3 bg-gray-800 border-b border-gray-700">
                    <div className="flex items-center space-x-2 sm:space-x-4">
                      <div className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-gradient-to-r ${platforms[activeTab].color}`}></div>
                      <div className="text-xs sm:text-sm text-gray-400">Social Dashboard</div>
                    </div>
                    <div className="flex space-x-2 sm:space-x-4">
                      <div className="w-3 h-1 sm:w-4 sm:h-1 rounded-full bg-gray-600"></div>
                      <div className="w-3 h-3 sm:w-4 sm:h-4 rounded-full bg-gray-600"></div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-1 sm:gap-2 p-2 sm:p-4">
                    <div className="bg-gray-800 p-2 sm:p-3 rounded-lg">
                      <div className="text-xs text-gray-400 mb-0.5 sm:mb-1">Posts</div>
                      <div className="text-sm sm:text-lg font-bold">24.8K</div>
                    </div>
                    <div className="bg-gray-800 p-2 sm:p-3 rounded-lg">
                      <div className="text-xs text-gray-400 mb-0.5 sm:mb-1">Reach</div>
                      <div className="text-sm sm:text-lg font-bold">478K</div>
                    </div>
                    <div className="bg-gray-800 p-2 sm:p-3 rounded-lg">
                      <div className="text-xs text-gray-400 mb-0.5 sm:mb-1">Growth</div>
                      <div className="text-sm sm:text-lg font-bold">+32%</div>
                    </div>
                  </div>
                </div>
                
                {/* Floating notification elements - responsive positioning and sizing */}
                <div className="hidden sm:block absolute -top-4 sm:-top-6 -right-4 sm:-right-6 bg-gray-900 border border-gray-800 rounded-lg p-2 sm:p-3 max-w-[120px] sm:max-w-[180px] shadow-xl transform translate-z-10">
                  <div className="flex items-center space-x-1 sm:space-x-2 mb-1 sm:mb-2">
                    <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-gradient-to-r ${platforms[activeTab].color}`}></div>
                    <div className="text-xs font-medium">{activeTab.charAt(0).toUpperCase() + activeTab.slice(1)}</div>
                  </div>
                  <div className="text-xs sm:text-sm">Your post reached 24,582 people in the last hour</div>
                </div>
                
                <div className="hidden sm:block absolute -bottom-3 sm:-bottom-4 -left-4 sm:-left-6 bg-gray-900 border border-gray-800 rounded-lg p-2 sm:p-3 shadow-xl transform translate-z-10">
                  ...
                  <div className="text-xs sm:text-sm">Engagement up 37% this week</div>
                </div>
              </div>
              ...
              <div className="absolute -z-10 inset-0 overflow-hidden">
                {[...Array(20)].map((_, i) => (
                  <div 
                    key={i}
                    className="absolute w-0.5 h-0.5 sm:w-1 sm:h-1 bg-white rounded-full opacity-30"
                    style={{
                      top: `${Math.random() * 100}%`,
